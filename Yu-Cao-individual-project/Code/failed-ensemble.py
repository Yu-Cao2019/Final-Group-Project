...
TEST_DIR = '/plant-pathology-2021-fgvc8/test_images'
...
with open('/model-conbination/pickle_model.pkl', 'rb') as file:
...
label = pickle_model.predict(datastack).tolist()
image = vpredictor.dfname


submission = pd.DataFrame({'image':image,'labels':label})
submission.labels=submission.labels.apply(lambda x: class_labels[x])
